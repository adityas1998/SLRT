...
class VideoDataset(data.Dataset):
    ...
    def _load_rgb(self, ind, frame_ix, modality = "rgb"):
        ...
        # video_file_path = 'psuedos/{:s}'.format(videofile.split('/')[-2]+'/'+videofile.split('/')[-1])
        # path = '../bsl/psuedos.zip' + '@' + video_file_path
        #
        # video_byte = ZipReader.read(path)
        # try:
        #     video_arrays = _load_frame_nums_to_4darray(video_byte, frame_ix) # T,H,W,3
        # except:
        #     print(path)
        # video = torch.tensor(video_arrays).float()
    ...
    def collate_fn(self, batch):
        ...
        meta = {"class", "index", "data_index", "dataset",'video_path'}
    ...
    def _get_single_video(self, index, data_index, frame_ix, modality):
        ...
        if getattr(self, "gpu_collation", False):
            ...
            data = {
                ...
                "index": index,
                "data_index": data_index,
                "class": self._get_class(index),
                "class_names": self.class_names,
                "video_path":self._get_video_file(index),
                "dataset": self.datasetname,
            }
            return data
